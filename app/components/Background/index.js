...
import head from './geometry/head.obj'
...
class Scene extends React.PureComponent {
  constructor(props) {
    ...
    this.loader.load(head, objData => {
      const obj = objData

      obj.traverse(childData => {
        const child = childData

        if (child instanceof THREE.Mesh) {
          if (child.name === 'head') child.material = this.mainMatcapMaterial
          else child.material = this.whiteMatcapMaterial
        }
      })
  ...
  handleMouseMove = e => {
    this.mouse = {
      start: true,
      x: (e.clientX - this.windowHalfX) * 0.7,
      y: (e.clientY - this.windowHalfY) * 0.5,
    }
  }
  ...
  componentDidMount() {
    ...
    this.addLine = (shape, pos, scale = 1) => {
      ...
      const shapeLine = new THREE.Line(
        shapeGeometry,
    ...
    this.addLine(
      backShape,
      [shapePos.x - 14, shapePos.y + 11, shapePos.z - 50],
      1.23,
    )
  ...
  animate = () => {
    // head rotation
    if (this.head) headRotation(this.head.rotation, this.mouse)
